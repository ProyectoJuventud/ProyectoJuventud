<?php
	...
	class EncuestasController extends AppController {
		...
		function add() {
			...
			$this->set('title_for_layout', 'Encuesta de las Juventudes Entrerrianas');

			$this->Session->delete('clave');
			$this->Session->write( 'clave');
			...
			if ($this->RequestHandler->isPost()) {
				...
				// Haremos el caso para si eligio NO. En PHP > 5.2 llega correcatmente "No" en el valor $this->data['Encuesta']['pregunta_10'].
				// Pero por alguna razon en PHP < 5.2 llega como "N". Entonces si llega "No" o "N" guardo "No".
				if(isset($this->data['Encuesta']['pregunta_10'])){

					if($this->data['Encuesta']['pregunta_10'] == 'No' || $this->data['Encuesta']['pregunta_10'] == 'N'){

						$this->data['Encuesta']['pregunta_10'] = 'No';
					}
				}

				// Si no entro a ninguno de los IF anteriores, es porque no existe porque en la pregunta 9 eligio SI y no se abrio la pregunta 10,
				// y dejara  VACIO.
				...
				if(isset($this->data['Encuesta']['pregunta_16'])){
					...
					if(is_array($this->data['Encuesta']['pregunta_16'])){
						// Convierto el array de opciones en un string donde cada elemento estara separado por una coma y este string
						// sera lo que grabemos en la base de datos.
				...
				if(isset($this->data['Encuesta']['pregunta_19']['Si'])){
					...
					if(is_array($this->data['Encuesta']['pregunta_19']['Si'])){
				...
				// Haremos el caso para si eligio NO. En PHP > 5.2 llega correcatmente "No" en el valor $this->data['Encuesta']['pregunta_20'].
				// Pero por alguna razon en PHP < 5.2 llega como "N". Entonces si llega "No" o "N" guardo "No".
				if(isset($this->data['Encuesta']['pregunta_20'])){

					if($this->data['Encuesta']['pregunta_20'] == 'No' || $this->data['Encuesta']['pregunta_20'] == 'N'){

						$this->data['Encuesta']['pregunta_20'] = 'No';
					}
				}

						////////////////////////////   PREGUNTA 21   /////////////////////////////

				if(isset($this->data['Encuesta']['pregunta_21'])){

					// Haremos el caso para si eligio NO. En PHP > 5.2 llega correcatmente "No" en el
					// valor $this->data['Encuesta']['pregunta_21'].
					// Pero por alguna razon en PHP < 5.2 llega como "N". Entonces si llega "No" o "N" guardo "No".
					if($this->data['Encuesta']['pregunta_21'] == 'No' || $this->data['Encuesta']['pregunta_21'] == 'N'){

						$this->data['Encuesta']['pregunta_21'] = 'No';
					}

					// Lo mismo ocurre con el "Si".
					if($this->data['Encuesta']['pregunta_21'] == 'Si' || $this->data['Encuesta']['pregunta_21'] == 'S'){

						$this->data['Encuesta']['pregunta_21'] = 'Si';
					}
				}
				...
				if(isset($this->data['Encuesta']['pregunta_22'])){
					...
					if(is_array($this->data['Encuesta']['pregunta_22'])){
				...
				if(isset($this->data['Encuesta']['pregunta_23'])){
					...
					if(is_array($this->data['Encuesta']['pregunta_23'])){
				...
				if(isset($this->data['Encuesta']['pregunta_27']['Si'])){
					...
					if(is_array($this->data['Encuesta']['pregunta_27']['Si'])){
				...
				if(isset($this->data['Encuesta']['pregunta_28']['Si'])){
					...
					if(is_array($this->data['Encuesta']['pregunta_28']['Si'])){
				...
				if(isset($this->data['Encuesta']['pregunta_30'])){
					...
					if(is_array($this->data['Encuesta']['pregunta_30'])){
				...
				if(isset($this->data['Encuesta']['pregunta_31'])){
					...
					if(is_array($this->data['Encuesta']['pregunta_31'])){
				...
				if(isset($this->data['Encuesta']['pregunta_34'])){
					...
					if(is_array($this->data['Encuesta']['pregunta_34'])){
				...
				// Haremos el caso para si eligio NO. En PHP > 5.2 llega correcatmente "No" en el valor $this->data['Encuesta']['pregunta_21'].
				// Pero por alguna razon en PHP < 5.2 llega como "N". Entonces si llega "No" o "N" guardo "No".
				if(isset($this->data['Encuesta']['pregunta_35'])){

					if($this->data['Encuesta']['pregunta_35'] == 'No' || $this->data['Encuesta']['pregunta_35'] == 'N'){

						$this->data['Encuesta']['pregunta_35'] = 'No';
					}
				}
				...
				if(isset($this->data['Encuesta']['pregunta_47']['Si'])){
					...
					if(is_array($this->data['Encuesta']['pregunta_47']['Si'])){
                ...
                if ($this->Encuesta->validates()) {
	                ...
	                if($this->Encuesta->save($this->data, false)){
	                   	...
	                   	$this->Session->setFlash('Encuesta completada correctamente, gracias!', 'flash_success');
	                   	$this->redirect(array('controller' => 'encuestas', 'action' => 'add'));
	                }
	                else{
	                	$this->Session->setFlash('No se han respondido todas las preguntas.', 'flash_failure');
	                }
	            }
		...
		function verificar_acceso(){
			...
			if ($this->RequestHandler->isPost()) {
				...
				if ($this->data['Acceso']['pass'] == 'Juventudes3510') {
					...
					$this->Session->setFlash('Acceso concedido', 'flash_success');
					$this->redirect(array('controller'=>'encuestas','action'=>'buscador'));
				}
		...
		function buscador(){
			...
			if($clave === 'Juventudes3510'){
				...
				if ($this->RequestHandler->isPost()) {
					...
					if ($this->data['Filtro']['filtro_edad'] != "") {

			            $condicion_edad = array('Encuesta.pregunta_1' => $this->data['Filtro']['filtro_edad']);

			            $this->Session->write('condicion_edad', $condicion_edad);
			        }
		            else{
		            	$this->Session->write('condicion_edad', 'true');
		            }


		            if ($this->data['Filtro']['filtro_sexo'] != "") {

			            $condicion_sexo = array('Encuesta.pregunta_2' => $this->data['Filtro']['filtro_sexo']);

			            $this->Session->write('condicion_sexo', $condicion_sexo);
		            }
		            else{
		            	$this->Session->write('condicion_sexo', 'true');
		            }


		            if ($this->data['Filtro']['filtro_zona_residencia'] != "") {

			            $condicion_zona_residencia = array('Encuesta.pregunta_4' => $this->data['Filtro']['filtro_zona_residencia']);

			            $this->Session->write('condicion_zona_residencia', $condicion_zona_residencia);
		            }
		            else{
	            		$this->Session->write('condicion_zona_residencia', 'true');
		            }


		            if ($this->data['Filtro']['filtro_departamento'] != "") {

	                    $condicion_departamento = array('Encuesta.pregunta_5' => $this->data['Filtro']['filtro_departamento']);

						$this->Session->write('condicion_departamento', $condicion_departamento);

						////////////////////////////
	                    // Pasaremos el value del Departamento seleccionado a la vista para luego agarrarlo desde Jquery y asignarlo en el SELECT.
	                    $condicion_departamento_value = reset($condicion_departamento);
						$this->set(compact('condicion_departamento_value'));
						////////////////////////////
		            }
		            else{
		            	$this->Session->write('condicion_departamento', 'true');
		            }


		   			// debug($this->Session->read('condicion_edad'));
					// debug($this->Session->read('condicion_sexo'));
					// debug($this->Session->read('condicion_zona_residencia'));
					// debug($this->Session->read('condicion_departamento'));

					// Leo las variables de sesion para darlas como condicion a la consulta
		            $condicion_edad = $this->Session->read('condicion_edad');
					$condicion_sexo = $this->Session->read('condicion_sexo');
					$condicion_zona_residencia = $this->Session->read('condicion_zona_residencia');
					$condicion_departamento = $this->Session->read('condicion_departamento');


					$encuestas = $this->Encuesta->find('all', array('recursive' => -1,
																    'conditions' => array($condicion_edad, $condicion_sexo,
																    					  $condicion_zona_residencia, $condicion_departamento
																    					)
																    )
					);
					...
					// Si no encontro ninguna encuensta, devuelvo 0
					if(count($encuestas) == 0){
						...
						$pregunta27_No = $pregunta27_Si = 0;

						////////////////////////////  PREGUNTA 27 (VARIABLES OPCIONES)  //////////////////////////
						$pregunta27_Si_opc1 = $pregunta27_Si_opc2 = $pregunta27_Si_opc3 = $pregunta27_Si_opc4 = $pregunta27_Si_opc5 =
						$pregunta27_Si_opc6 = $pregunta27_Si_opc7 = 0;

						////////////////////////////  PREGUNTA 27 OTRAS (VARIABLES)  //////////////////////////
						//$pregunta27_otras = [];

						////////////////////////////  PREGUNTA 28 (VARIABLES)  //////////////////////////
						$pregunta28_No = $pregunta28_Si = 0;

						////////////////////////////  PREGUNTA 28 (VARIABLES OPCIONES)  //////////////////////////
						$pregunta28_Si_opc1 = $pregunta28_Si_opc2 = $pregunta28_Si_opc3 = $pregunta28_Si_opc4 = $pregunta28_Si_opc5 =
						$pregunta28_Si_opc6 = $pregunta28_Si_opc7 = 0;
						...
						foreach($encuestas as $encuesta){
							...
							if($encuesta['Encuesta']['pregunta_27'] == 'No'){

								$pregunta27_No++;
							}

							// Si entra es porque hay array de numeros (opciones), primero incremento el SI y despues segun las opciones
							// elegidas voy incrementando.
							else{
							...
							if($encuesta['Encuesta']['pregunta_28'] == 'No'){
						...
						// SI la variable esta existe (solo existe si alguien ha escrito algo), entonces lo manda a la vista
						if(isset($pregunta19_otras)){
						...
						$this->set(compact('pregunta27_No','pregunta27_Si'));

						$this->set(compact('pregunta27_Si_opc1', 'pregunta27_Si_opc2', 'pregunta27_Si_opc3', 'pregunta27_Si_opc4',
										   'pregunta27_Si_opc5', 'pregunta27_Si_opc6', 'pregunta27_Si_opc7'));

						if(isset($pregunta27_otras)){
							$this->set(compact('pregunta27_otras'));
						}

						$this->set(compact('pregunta28_No','pregunta28_Si'));

						$this->set(compact('pregunta28_Si_opc1', 'pregunta28_Si_opc2', 'pregunta28_Si_opc3', 'pregunta28_Si_opc4',
										   'pregunta28_Si_opc5', 'pregunta28_Si_opc6', 'pregunta28_Si_opc7'));

						if(isset($pregunta28_otras)){
				...
				// Si no es POST
					...
					// la unica forma que no sea post es cuando entra por primera vez. Por lo tanto, borro el contenido de las
					// variable s de sesion y las vuelvo a declarar.
					$this->Session->delete('condicion_edad');
					$this->Session->write( 'condicion_edad');

					$this->Session->delete('condicion_sexo');
					$this->Session->write( 'condicion_sexo');

					$this->Session->delete('condicion_zona_residencia');
					$this->Session->write( 'condicion_zona_residencia');

					$this->Session->delete('condicion_departamento_value');
					$this->Session->write( 'condicion_departamento_value');
